...
export interface CursorChangeEvent {
  type: 'cursor_change';
  file: string;
  selections: Array<{ start: { line: number; character: number }; end: { line: number; character: number }; isReversed: boolean }>;
}
...
export interface TextEditCommand {
  ...
  command: 'edit_text';
  file: string;
  edits: Array<{ range: { startLine: number; startCharacter: number; endLine: number; endCharacter: number }; text: string }>;
  id?: string;
}
